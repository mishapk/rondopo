...
procedure  FormatZapr_X305( Adress,  funct,  subfunct:integer); stdcall; external 'dozor.dll';
 ...
 function GetPi:integer; stdcall; external 'dozor.dll';
      ...
      begin
         with AddChild('Date') do
           begin
             ...
             fl:=0;
             m1:      //Цикл проверки точности результата
             //---------------X305---------------------------------------------
               FormatZapr_X305($10,$04,$00);
               ResultZapr_X305(a,b,c,d);
               sleep(100);
               if(d<>0)and (fl=0) then begin sleep(200);fl:=1; goto m1 end;
             //-----------------------------------------------------------------

              cs:=ReadBitX305(d);
             if debug then FormMain.CAPTION_TEXT:=TimeToStr(Now)+' '+cs+ ' '+ BoolToStr(debug);
             //  FormMain.Caption:=cs;
          //    showmessage(cs+'Hi');
             for i:=1 to 6 do
               with AddChild('gauge') do
...
var st,sb:string;
    command:integer;
begin
    if Debug Then   FormMain.CAPTION_TEXT:='Pause 2SEC!!';
         sleep(25);
   st:=global_st;
