...
begin
  ...
  // Обновление Датчиков
  s := 'select SENSORS.MkuКод, SENSORS.Код, SENSORS.SensorID, SENSORS.SensorType, SENSORS.SensorParameter, SENSORS.SensorPlace from SENSORS where SENSORS.MkuКод ='
    + dsMkus.DataSet.Fields.FieldByName('Код').AsString + ' ORDER BY Код ASC';
...
begin
  ...
  // Выход, если Датчиков нет
  if dsSensors.DataSet.RecordCount = 0 then
    exit;

  s := 'select BLOCKS.Код, BLOCKS.BlockID, BLOCKS.Danger, BLOCKS.Описание from BLOCKS, LINKS where LINKS.SensorКод='
    + dsSensors.DataSet.Fields.FindField('Код').AsString +
    ' and LINKS.BlockКод = BLOCKS.Код ORDER BY BLOCKS.BlockID ASC';
  // ShowMessage(s);
...
begin
  ...
  // выход, если невыбраны МКУ или Сенсоры
...
begin
  ...
  begin
    ...
    s := 'delete * from LINKS where BlockКод = ' +
      dsBlocks.DataSet.Fields.FieldByName('Код').AsString + ' and SensorКод=' +
      dsSensors.DataSet.Fields.FieldByName('Код').AsString;
