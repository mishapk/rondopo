...
begin
  ...
  qLinks.Active := False;
  qLinks.SQL.Text := 'select Код from BLOCKS where BLOCKS.BlockID = ' +
    ComboBox1.Text + ' and BLOCKS.ObjectКод = ' +
    FormLinks.dsMkus.DataSet.Fields.FindField('ObjectКод').AsString;
  qLinks.Active := True;

  qSave.Active := False;
  qSave.SQL.Text :=
    'insert into LINKS (SensorКод, BlockКод) VALUES (:SensorКод, :BlockКод)';

  qSave.Parameters.Items[0].Value := FormLinks.dsSensors.DataSet.Fields.
    FindField('Код').AsString;
...
begin
  ...
  qLinks.Active := False;
  qLinks.SQL.Text := 'select Описание from BLOCKS where BLOCKS.BlockID = ' +
    ComboBox1.Text + ' and BLOCKS.ObjectКод = ' +
    FormLinks.dsMkus.DataSet.Fields.FindField('ObjectКод').AsString;
  qLinks.Active := True;

  Memo1.Text := dsLinks.DataSet.FieldByName('Описание').AsString;
...
begin
  ...
  qLinks.Active := False;
  qLinks.SQL.Text := 'select * from BLOCKS where BLOCKS.ObjectКод = ' +
    FormLinks.dsMkus.DataSet.Fields.FindField('ObjectКод').AsString +
    ' ORDER BY BLOCKS.BlockID ASC';
  qLinks.Active := True;

  // если блоков нету то выход
  if qLinks.RecordCount <= 0 then
  begin
    ShowMessage('Сначала добавьте блоки опасности');
    Close;
  end;
