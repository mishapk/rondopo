...
begin
  ...
  ADOQuery1.SQL.Add
    ('FROM (TYPESEN INNER JOIN SEN_SET ON TYPESEN.Код = SEN_SET.TypeKOD) INNER JOIN (SENSORS INNER JOIN ARCHIVE ON SENSORS.Код = ARCHIVE.KOD_SENSOR) ON TYPESEN.Код = SENSORS.SensorType');
...
begin
  AdvOfficeStatusBar1.Panels.Items[1].Text := 'Всего записей:' +
    IntToStr(ADOQuery1.RecordCount);
  AdvOfficeStatusBar1.Panels.Items[0].Text := 'Номер записи:' +
...
begin
{
  ...
  XMLtoGRID;     }
  count:=count+1;
  SG.Cells[3, 1] := 'Связь c Дозор-1';
 if count>1 then
  SG.Cells[4, 1] := '2';
  SG.Cells[3, 2] := 'Модуль В.В.';
  if count>2 then
  SG.Cells[4, 2] := '2';
  SG.Cells[3, 3] := 'ГГС-1';
  if count>3 then
  ...
  if count>7 then
  SG.Cells[4, 7] := '1';
   SG.Cells[3, 8] := 'Связь с БЗО';
   if count>8 then
  SG.Cells[4, 8] := '2';
   SG.Cells[3, 9] := 'БЗО Контролер ';
   if count>9 then
  SG.Cells[4, 9] := '2';
  SG.Cells[3, 10] := 'БЗО GSM-1';
  if count>10 then
  SG.Cells[4, 10] := '2';
  SG.Cells[3, 11] := 'БЗО GSM-2';
  if count>11 then
  SG.Cells[4, 11] := '1';
  SG.Cells[3, 12] := 'БЗО ГГС-1';
  if count>12 then
  SG.Cells[4, 12] := '2';
  SG.Cells[3, 13] := 'БЗО Питание';
  if count>13 then
  SG.Cells[4, 13] := '2';
  SG.RowCount:=14;
